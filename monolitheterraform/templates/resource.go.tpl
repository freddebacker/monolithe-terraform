...
import (
    ...
    {%- if (specification.attributes | selectattr('type', 'equalto', 'object') | list | length) > 0 %}
    "reflect"
    {%- endif %}
)
    ...
    return &schema.Resource{
        ...
        Schema: map[string]*schema.Schema{
            ...
            "{{ attribute.local_name|lower }}{{ "_" if attribute.local_name.lower() in ["connection", "count", "depends_on", "id", "lifecycle", "provider", "provisioner"] }}": &schema.Schema{
                ...
                {%- if attribute.required and not (attribute.type == "object" or attribute.autogenerated or attribute.read_only) %}
                Required: true,
                ...
                ConflictsWith: []string{"{{ attribute.local_name|lower }}_string"},
                DiffSuppressFunc: func(k, oldval, newval string, d *schema.ResourceData) bool {
                    m := make(map[string]string)
                    m["string"] = newval
                    if reflect.DeepEqual(oldval, m) {
                        return true
                    }
                    return false
                },
                ...
                {%- endif %}
                {%- if attribute.default_value and attribute.type != "list" and not (attribute.required  or attribute.autogenerated or attribute.read_only)%}
                Default: {{ get_default_value(attribute) }},
                {%- endif %}
            },
            {%- if attribute.type == "object" %}
            "{{ attribute.local_name|lower }}_string": {
				Type:     schema.TypeString,
				Optional: true,
                Computed: true,
                ConflictsWith: []string{"{{ attribute.local_name|lower }}"},
			},
            {%- endif %}
            {%- endfor %}
...
func resource{{specification.entity_name}}Create(d *schema.ResourceData, m interface{}) error {
    ...
    o := &vspk.{{ specification.entity_name }}{
        ...
        {{ field_name }}: d.Get("{{ attribute.local_name|lower }}{{ "_" if attribute.local_name.lower() in ["connection", "count", "depends_on", "id", "lifecycle", "provider", "provisioner"] }}").({{ attribute.local_type }}),
        {%- endif %}
        {%- endfor %}
    }
...
func resource{{specification.entity_name}}Read(d *schema.ResourceData, m interface{}) error {
    ...
    if v, ok := o.{{ field_name }}.(string); ok {
		...
		mp["string"] = v
		d.Set("{{ attribute.local_name|lower }}_string", v)
		d.Set("{{ attribute.local_name|lower }}", mp)
	} else {
		d.Set("{{ attribute.local_name|lower }}{{ "_" if attribute.local_name.lower() in ["connection", "count", "depends_on", "id", "lifecycle", "provider", "provisioner"] }}", o.{{ field_name }})
	}
    {%- endif %}
